...
const client = await bootstrap({
  ...
  native: {
    ...
    // On macOS, set directory to the installed nativeRuntimeDirectory.
    settings: {
...
The bundled Windows runtime remains available with `npm install --omit=optional`. Only Windows managed mode resolves and starts it. macOS consumers also receive these bytes but do not execute them; the macOS CLI separately installs the selected Native SDK Runtime. Mixed Mode still uses its application-owned native host. The bundled Windows runtime covers x64 only; other architectures require an explicit compatible build.
...
The Native SDK version is required; the CLI has no default version. The command downloads, verifies, and installs the corresponding Universal runtime for Apple Silicon and Intel Macs in `.cloudcare/native/darwin/runtime`. Pass this directory as `native.directory` during development. When packaging the application, copy the complete directory outside ASAR and sign its Native binary with the application.
...
For local development or offline installation, use an archive generated by the Native SDK and its adjacent SHA-256 sidecar. The specified version must match the archive manifest:

    npx guance-electron-native --sdk-version <version> --runtime-archive /path/to/guance-electron-runtime-<version>-darwin-universal.tar.gz
